...
parser.add_argument("epochs_number",type=int,help="Number of epochs in training")
parser.add_argument("train_path",type=str,help="Path to save training")
...
class MyClassificationTrainer(ClassificationTrainer):
    def __init__(self, model: nn.Module, trainer_k:str=None, num_classes=None, lr: float = 0.0, weight_decay: float = 0.0, optimizer=None, **kwargs):
...
if __name__ == '__main__':
    ...
    dataset = SEEDIVFeatureDataset(io_path=f'./dataset/seed_iv', root_path='./dataset/eeg_feature_smooth',
                                   feature=['de_LDS'],
                                   online_transform=transforms.Compose([
                                       transforms.MeanStdNormalize(),
                                       ToG(SEED_IV_STANDARD_ADJACENCY_MATRIX)
    ...
    k_fold = KFoldGroupbyTrial(n_splits=5,shuffle=True,random_state=10,split_path='./dataset/splits')

    # Addestramento della rete neurale
    for i, (train_dataset, val_dataset) in track(enumerate(k_fold.split(dataset)), "[bold green]Training: ", total=5):
        model = RGNN(adj=torch.Tensor(SEED_IV_STANDARD_ADJACENCY_MATRIX), in_channels=2, num_classes=4, dropout = 0.7,  learn_edge_weights = True).to(device)
        trainer =  MyClassificationTrainer(model=model, trainer_k = i,optimizer = Adam(model.parameters(),lr = 0.0001))
